...
### 七、去AI痕迹（违反任何一条即为失败）
**以下表达是AI生成小说的典型套话，你必须绝对禁止在文中使用。如果出现了这些表达，你的作品将被判定为AI生成并废弃。**

**绝对禁止的套话清单（出现即判废）：**
- "那是传说中的" / "传说中的" —— 改为"老辈人提过，真假说不清"
- "眼中燃烧着不屈的火焰" / "眼中闪烁着坚定的光芒" —— 改为具体神态描写（"眼神变了""目光沉下去"）
- "握紧了拳头" / "攥紧了拳头" —— 改为"指节发白""指甲嵌进掌心"
- "成为强者，这是他此生唯一的目标" —— 改为"变强这件事，他没想过为什么，只是非做不可"
- "无论前方有多少艰难险阻，他都不会退缩" —— 改为"路不好走，但停下来更难受"
- "因为，他早已没有退路" —— 改为"不是不想退，是没地方退了"
- "在这一刻，他仿佛听到了命运的召唤" —— 改为"心里忽然空了一下，说不清是什么感觉"
- "心跳快得像要跳出胸腔" —— 改为"胸口闷得厉害""心口一下一下撞着肋骨"
- "每一根神经都绷紧到了极致" —— 改为"后背发凉""头皮发麻"
- "危险正在逼近" —— 改为"空气里有什么东西变了""他忽然觉得冷"
- "在这千钧一发之际" —— 改为"眼看就要出事""来不及细想了"
- "生，或者死" —— 改为"不是赢就是输，很简单"
- "风呼啸而过，吹动他的衣袂" —— 改为"风吹过来，衣服贴在背上"
- "远方的天际线上" —— 改为"天边"
- "一道金光冲天而起" —— 改为"天上忽然亮了亮"
- "云海翻涌" —— 改为"云在脚底下动"
- "时间所剩无几" —— 改为"没多少时间了"
...
- 禁止连续3句以上结构相似的排比句
- 禁止"首先……其次……最后……"三段式
- 禁止"不仅……而且……""一方面……另一方面……"等AI对称句式
- 禁止连续使用相同长度的句子（必须长短交错）
- 禁止所有"作为……的证明""标志着""为……奠定基础""具有重要意义"

**语言风格要求：**
- 用口语化、不完美的表达替代书面语（"嗯""妈的""说白了""就是""怎么说呢"）
- 允许省略主语的短句，允许不完整的句子
- 句子长度必须剧烈变化：短句3-10字，长句25-50字交替出现
- 用感官细节替代情绪标签：不写"他很愤怒"，写"指节发白、呼吸变重、摔了杯子"
- 用动作承接代替逻辑连接：不写"因为他很生气，所以摔了门"，写"他摔了门。外面下雨了。"
...
{content}"""
            ...
            try:
                ...
                if deai and len(content) > 100:
                    from ai_humanizer import detect_ai_traces, humanize_text
                    detection = detect_ai_traces(content)
                    ai_score = detection.get('total_score', 0)
                    log(f"AI detection score: {ai_score}")
                    # score > 50 表示AI痕迹较重，需要后处理去AI
                    if ai_score > 50:
                        log(f"Auto DEAI triggered (score={ai_score}), applying humanize...")
                        content = humanize_text(content)
                        log(f"DEAI applied, new length: {len(content)}")
                    else:
                        log(f"Auto DEAI skipped (score={ai_score}), AI痕迹已通过生成prompt控制")
