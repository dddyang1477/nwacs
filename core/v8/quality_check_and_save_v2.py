...
class QualityChecker:
    ...
    def run_all_checks(self):
        ...
        print(f"\n{'='*60}")
        print(f"{'✅ 检测通过' if final_result else '❌ 检测不通过'}")
        print(f"{'='*60}")
    ...
    def check_readability(self):
        ...
        passed = 0.03 < punc_ratio < 0.08
        
        msg = f"{'✅' if passed else '⚠️'} 标点密度: {punc_ratio:.2%}"
        return passed, msg
    ...
    def check_ending(self):
        ...
        msg = f"{'✅' if passed else '⚠️'} 结尾: {len(last_part)}字"
        return passed, msg
...
def mark_for_manual_check(chapter_num, report):
    """
    标记为需要人工处理
    - 创建待处理标记文件
    - 记录检测报告
    ...
    # 1. 创建标记文件
    mark_filename = f"novel/第{chapter_num}章_需要人工检查.txt"
    
    warning_content = f"""⚠️  WARNING: 此章节需要人工检查 ⚠️
{'='*70}
生成时间: {timestamp}
章节: 第{chapter_num}章
...
"""
    ...
    print(f"   ⚠️ 已更新待处理列表: {log_file}")

def generate_and_check(chapter_num, outline):
    """
    完整流程
    """
    print(f"\n🚀 开始处理第{chapter_num}章")
...
直接输出章节内容，不要标题！"""
        ...
        if passed:
            print(f"\n💾 检测通过！正在保存...")
            filename = save_chapter(chapter_num, content)
            ...
            if attempt < MAX_RETRY:
                ...
                print(f"\n{'='*70}")
                print(f"⚠️ ⚠️ ⚠️  WARNING: 第{attempt}次检测全部失败！")
                print(f"{'='*70}")
                ...
                mark_for_manual_check(chapter_num, last_report)
                
                filename = save_chapter(chapter_num, content)
                print(f"⚠️ 已强制保存: {filename}")
...
def main():
    ...
    print(f"✅ 最终字数: {len(content)}字")
    
    if success:
        print(f"🎉 检测通过，完美！")
    else:
        print(f"⚠️ 检测未通过，已标记待人工处理！")
