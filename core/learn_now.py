...
class AutoLearningSystem:
    ...
    def learn_foreign_classics(self):
        classics = {
            "希腊神话": ["宙斯", "赫拉", "波塞冬", "雅典娜", "阿波罗", "战神阿瑞斯"],
            "罗马神话": ["朱庇特", "朱诺", "尼普顿", "维纳斯", "玛尔斯"],
            "圣经故事": ["创世纪", "出埃及记", "大卫与歌利亚", "耶稣诞生"],
            "格林童话": ["灰姑娘", "白雪公主", "小红帽", "睡美人"],
            "安徒生童话": ["丑小鸭", "卖火柴的小女孩", "海的女儿", "皇帝的新装"],
            "日本神话": ["天照大神", "八岐大蛇", "桃太郎", "辉夜姬"],
            "印度神话": ["梵天", "毗湿奴", "湿婆", "罗摩衍那"],
            "哈利波特": ["魔法世界观", "霍格沃茨", "魔杖系统", "巫师等级"]
        }
        
        self.learned_content["国外经典"] = classics
        
        for name, items in classics.items():
            print(f"  🌍 研究：{name}")
            content = f"【国外经典】{name}\n\n"
            content += "核心元素：" + "、".join(items) + "\n"
            content += f"\n学习时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            self._save_learning(content, "国外经典")
            time.sleep(0.2)
        print(f"  ✅ 完成国外经典学习")
