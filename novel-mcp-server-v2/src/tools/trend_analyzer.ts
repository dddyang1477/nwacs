...
export async function analyzeTrend(params: TrendAnalyzerParams): Promise<TrendAnalysisResult> {
  try {
    const { platform, genre, time_range = 'month' } = params;
    
    const hot_topics: Record<string, string[]> = {
    ...
    return {
      ...
      market_data: {
        ...
        competition_level: (genre === 'xuanhuan' || genre === 'urban' ? 'high' : 'medium') as string
      }
    };
  } catch (error) {
    return {
