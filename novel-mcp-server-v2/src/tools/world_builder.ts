import type { WorldBuilderParams, WorldBuilderResult } from '../types/index.js';

export async function operateWorld(params: WorldBuilderParams): Promise<WorldBuilderResult> {
  try {
    const { action, novel_id, content, dimension = 'geography' } = params;

    const worldTemplates: Record<string, Record<string, string>> = {
      geography: {
