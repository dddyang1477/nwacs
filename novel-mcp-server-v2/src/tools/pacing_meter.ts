import type { PacingAnalysisParams, PacingAnalysis } from '../types/index.js';

export async function analyzePacing(params: PacingAnalysisParams): Promise<PacingAnalysis> {
  try {
    ...
    const dialogueLength = dialogueMatches.reduce((sum: number, m: string) => sum + m.length, 0);
