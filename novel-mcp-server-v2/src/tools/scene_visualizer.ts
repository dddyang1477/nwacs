import type { SceneVisualizerParams, SceneVisualization } from '../types/index.js';

export async function enhanceVisualization(params: SceneVisualizerParams): Promise<SceneVisualization> {
  try {
    ...
    const sensory_enhancements: Record<string, Record<string, string[]>> = {
      visual: {
    ...
    if (enhancement_level !== 'light') {
      ...
      const enhancementPhrases: string[] = [];
      if (visuals.length > 0) enhancementPhrases.push(visuals[Math.floor(Math.random() * visuals.length)]);
      if (audios.length > 0) enhancementPhrases.push(audios[Math.floor(Math.random() * audios.length)]);
