...
from core.v8.platform_exporter import (
    PlatformExporter, NovelMeta, ChapterData, ExportPlatform,
    QualityCheckResult, QualityCheckFailedException, MIN_CHAPTER_WORDS,
)
...
result = pe.export(meta, chapters, ExportPlatform.QIDIAN, skip_quality_check=True)
check("导出起点(跳过质检)", result is not None)

result = pe.export(meta, chapters, ExportPlatform.TXT, skip_quality_check=True)
check("导出TXT(跳过质检)", result is not None)
...
# 质量检测-基础验证（不调用API）
qc_result = pe.check_quality_before_export(meta, chapters)
check("质量检测-字数不足拦截", not qc_result.passed and len(qc_result.failed_chapters) == 2)
check("质量检测-综合评分", qc_result.overall_score < 100)
check(f"质量检测-最低字数要求{MIN_CHAPTER_WORDS}字", MIN_CHAPTER_WORDS == 4000)

# 导出文件夹测试
safe_title = pe._safe_filename(meta.title)
novel_dir = os.path.join(TEST_DIR, safe_title)
check("小说文件夹已创建", os.path.isdir(novel_dir))


# ================================================================
# 5b. QualityDetector 质量检测师模块测试
# ================================================================
print("\n" + "=" * 60)
print("5b. QualityDetector 质量检测师测试")
print("=" * 60)

from core.v8.quality_detector import (
    QualityDetector, AITraceReport, QualityReport,
    PipelineResult, AI_TRACE_THRESHOLD, QUALITY_THRESHOLD,
)

qd = QualityDetector()
check("初始化", qd is not None)

test_text = "林晨缓缓地站起身，宛如一只刚刚破茧而出的蝴蝶，心中不禁激动万分。仿佛天地都在为他欢呼，眼前的景象十分壮观，格外震撼人心。"

ai_report = qd.detect_ai_traces(test_text)
check("AI痕迹检测", isinstance(ai_report, AITraceReport))
check("AI痕迹分数有效", 0 <= ai_report.score <= 100)
check("AI痕迹等级有效", ai_report.level in ("low", "medium", "high"))
check("AI痕迹置信度有效", 0 <= ai_report.confidence <= 1)

quality_report = qd.evaluate_quality(test_text)
check("质量评估", isinstance(quality_report, QualityReport))
check("质量分数有效", 0 <= quality_report.score <= 100)
check("结构分数有效", 0 <= quality_report.structure_score <= 100)
check("可读性分数有效", 0 <= quality_report.readability_score <= 100)
check("文学性分数有效", 0 <= quality_report.literary_score <= 100)
check("连贯性分数有效", 0 <= quality_report.coherence_score <= 100)

check(f"AI阈值={AI_TRACE_THRESHOLD}", AI_TRACE_THRESHOLD == 40)
check(f"质量阈值={QUALITY_THRESHOLD}", QUALITY_THRESHOLD == 60)
...
exported = pe2.export(meta2, ch2, ExportPlatform.TXT, skip_quality_check=True)
