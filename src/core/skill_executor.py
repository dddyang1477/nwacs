...
try:
    ...
    NOVEL_WRITER_AVAILABLE = True
    logger.info("NovelWriter工具加载成功")
except ImportError as e:
    NOVEL_WRITER_AVAILABLE = False
    logger.warning(f"NovelWriter工具加载失败: {e}")
...
class SkillExecutorV2:
    ...
    def __init__(self):
        ...
        # 初始化novel-writer工具
        self.novel_writer = None
        self._init_novel_writer()
    ...
    def _init_novel_writer(self):
        """初始化novel-writer工具"""
        if NOVEL_WRITER_AVAILABLE:
            try:
                self.novel_writer = NovelWriter()
                logger.info("NovelWriter工具初始化成功")
            except Exception as e:
                logger.log_exception(e, "初始化NovelWriter失败")
                self.novel_writer = None
        else:
            logger.warning("NovelWriter工具不可用")
    
    def execute_novel_writer(self, action, **kwargs):
        """执行novel-writer工具操作"""
        if not self.novel_writer:
            return {'status': 'failed', 'error': 'NovelWriter工具未初始化'}
        ...
        try:
            if action == 'generate_outline':
                ...
                result = self.novel_writer.generate_outline(genre, theme, length)
                return {'status': 'completed', 'result': result}
            
            elif action == 'generate_chapter':
                chapter_info = kwargs.get('chapter_info', {'chapter': 1, 'title': '第一章', 'content': '开端'})
                outline = kwargs.get('outline', {})
                result = self.novel_writer.generate_chapter(chapter_info, outline)
                return {'status': 'completed', 'result': result}
            
            elif action == 'polish_text':
                text = kwargs.get('text', '')
                style = kwargs.get('style', '流畅')
                result = self.novel_writer.polish_text(text, style)
                return {'status': 'completed', 'result': result}
            
            elif action == 'save_novel':
                novel_data = kwargs.get('novel_data', {})
                filename = kwargs.get('filename', None)
                result = self.novel_writer.save_novel(novel_data, filename)
            ...
            logger.log_exception(e, f"execute_novel_writer({action})")
